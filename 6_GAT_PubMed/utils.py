...
def load_data(miss_1_or_2, path="./datasets/pubmed/", dataset="pubmed"):
    ...
    if miss_1_or_2 == 0:
        ...
        laplace_a = sp.coo_matrix((np.ones(aa.shape[0]), (aa[:, 0], aa[:, 1])),  # (edges[:, 0], edges[:, 1])这些位置的值为1
        ...
        laplace_b = sp.coo_matrix((np.ones(bb.shape[0]), (bb[:, 0], bb[:, 1])),  # (edges[:, 0], edges[:, 1])这些位置的值为1
                                  shape=(labels.shape[0], labels.shape[0]),
                                  dtype=np.float32)
