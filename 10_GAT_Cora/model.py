...
class GraphConvolution(nn.Module):
	...
	def forward(self, x, adj):
		...
		relationship = torch.mm(support, support.T)
		relationship = (relationship-relationship.min())/(relationship.max()-relationship.min())
		adj_numpy = np.int64(adj.detach().numpy() > 0.5)
		row, col = np.diag_indices_from(adj_numpy)
		adj_numpy[row, col] = 1
		adj_relationship = adj_numpy * relationship.detach().numpy()

		row_sum = np.array(adj_relationship.sum(1))  # 对每一行求和
		r_inv = np.power(row_sum, -1).flatten()  # 求倒数
		r_inv[np.isinf(r_inv)] = 0.  # 如果某一行全为0，则r_inv算出来会等于无穷大，将这些行的r_inv置为0
		r_mat_inv = sp.diags(r_inv)  # 构建对角元素为r_inv的对角矩阵
		adj_softmax = r_mat_inv.dot(adj_relationship)
		adj_softmax = torch.tensor(adj_softmax)
		adj_softmax = adj_softmax.float()

		# row_sum = np.array(adj.sum(1))  # 对每一行求和
		# r_inv = np.power(row_sum, -1).flatten()  # 求倒数
		# r_inv[np.isinf(r_inv)] = 0.  # 如果某一行全为0，则r_inv算出来会等于无穷大，将这些行的r_inv置为0
		# r_mat_inv = sp.diags(r_inv)  # 构建对角元素为r_inv的对角矩阵
		# adj_n = r_mat_inv.dot(adj)
		# adj_n = torch.tensor(adj_n)

		output = torch.mm(adj_softmax, support)
		if self.bias is not None:
