...
def load_data(miss_1_or_2, path="./datasets/citeseer/", dataset="citeseer"):
    ...
    if miss_1_or_2 == 0:
        ...
        # creat_miss = np.random.randint(0, 100, 2708)
        # miss_2 = np.int64(creat_miss > 15)
        miss_2 = np.ones(nodes_count) - miss_1
        ...
        laplace_up = np.dot(sp.diags(miss_1).A, laplace,)
        laplace_down = np.dot(sp.diags(miss_2).A, laplace)
        laplace_up = torch.FloatTensor(laplace_up)
        laplace_down = torch.FloatTensor(laplace_down)
